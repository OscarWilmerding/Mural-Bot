/****************************************************/
/*    Single-Block Code Using Struct for Commands   */
/****************************************************/
...
/*
  ...
  5) After the code, there is an explanation of why a struct is helpful/necessary.
*/
...
void determineStripeVelocities() {
  //positions in meters for each stepper
...
void startNextCommand() {
  if (currentCommandIndex < commandCount) {
    ...
    if (cmd.type == Command::MOVE) {
      ...
      float posA = 0.0, posB = 0.0;
      determinePulleyPositionsStripe(cmd, posA, posB);
